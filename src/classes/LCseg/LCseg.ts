...
import { SentenceObject, UtteranceObject } from '../../interfaces/DebateDataInterface';
...
export default class LCseg {
	...
	constructor(
		...
		_.forEach(utteranceObjects, (utteranceObject, utteranceIndex) => {
			_.forEach(utteranceObject.sentenceObjects, (sentenceObject, sentenceIndex) => {
				this.totalSentenceObjects.push(sentenceObject);
				this.sentenceIndexUtteranceIndexDict[this.totalSentenceObjects.length - 1] = utteranceIndex;
			});
		});

		// make probabilities from start until end sentence index
		for (let i = 2; i < this.totalSentenceObjects.length - 2; i++) {
			this.segmentationProbabilities.push(this.makeSegmentationProability(i, termList));
		}
		console.log('this.segmentationProbabilities', this.segmentationProbabilities);
	}
	...
	private lexicalCohesionFunction(
		...
		function wTermAndWindow(
			...
			if (term in termLexicalChainDictOfWindow) {
				return this.scoreRi(term, termIndex, this.frequencyVector, this.totalSentenceObjects);
			} else {
				return 0;
			}
		}
		...
		function makeCombinedTermLexicalChainDict(
			...
			_.forEach(sentenceIndicesOfWindow, (sentenceIndex) => {
				_.forEach(sentenceIndexLexicalChainsDict[sentenceIndex], (lexicalChain, term) => {
	...
	private scoreRi(
		...
		const score: number =
			frequencyVectorOfTerms[termIndex] * Math.log(totalSentenceObjects.length / Li);
		return score;
	}

	private makeSegmentationProability(sentenceIndex: number, termList: string[]): number {
