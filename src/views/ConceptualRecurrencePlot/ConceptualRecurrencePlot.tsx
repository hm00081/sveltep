...
import ConceptualMapModal, { ConceptualMapModalRef } from './ConceptualMapModal/ConceptualMapModal';
...
import { DebateDataSet, EvaluationDataSet } from '../../interfaces/DebateDataInterface';
...
function ConceptualRecurrencePlot() {
	...
	const [debateDataset, setDebateDataset] = useState<DebateDataSet | null>(null);
	const [dataStructureManager, setDataStructureManager] = useState<DataStructureManager | null>(
		null
	);
	const [evaluationDataSet, setEvaluationDataSet] = useState<EvaluationDataSet | null>(null);

	const [combinedEGsMaker, setCombinedEGsMaker] = useState<CombinedEGsMaker | null>(null);
		...
		if (!dataStructureManager) {
			if (
				...
				dispatch({
					...
					payload: {
						standardSimilarityScore: dataStructureMaker.dataStructureSet.maxSimilarityScore
					}
				});
	...
	useEffect(() => {
		if (dataStructureManager && debateDataset) {
			...
			const conceptSimilarityBlocks = dataStructureSet.similarityBlockManager.similarityBlocks;
			const conceptSimilarityMatrix = dataStructureSet.similarityBlockManager.similarityBlockGroup;
			...
			const d3Drawer = new D3Drawer(debateDataset, dataStructureSet, termTypeOfQuery);
	...
	return (
		<div className="root-div">
			<Controllers
				...
				maxSimilarityScore={
					dataStructureManager ? dataStructureManager.dataStructureSet.maxSimilarityScore : 0
				}
				debateDataset={debateDataset}
				evaluationDataSet={evaluationDataSet}
				dataStructureSet={dataStructureManager ? dataStructureManager.dataStructureSet : null}
				dataStructureManager={dataStructureManager ? dataStructureManager : null}
			></Controllers>
			...
			<TranscriptViewer dataStructureMaker={dataStructureManager}></TranscriptViewer>

			<ConceptualMapModal
				ref={conceptualMapModalRef}
				participantDict={
					dataStructureManager ? dataStructureManager.dataStructureSet.participantDict : {}
				}
				...
				termUtteranceBooleanMatrixTransposed={
					debateDataset ? debateDataset.termUtteranceBooleanMatrixTransposed : []
				}
