...
interface Script {
	...
	talkerId: string;
	text: string;
	startTime: number; // 현재 객체의 time
	endTime: number; // 이전 객체의 time
	agenda: number;
}
...
export default class Narrative {
	// public : 클래스 내부, 자식 클래스, 클래스 인스턴스 모두 접근 가능.
	// private: 클래스 내부만 접근 가능. (자식, 클래스 인스턴스 x)
	...
	private clickTopicKeyword(topic: Topic, topicIndex: number) {
		...
		this.displayScriptItems = this.findScriptItems([findingNode], this.scriptsByTopic[topicIndex]);
	}
	//@ts-ignore
	...
	private findScriptItems(clickedNodes: Node[], scripts: Script[]) {
		...
		return this.getScriptItem(scripts, highlightScriptIndexs, clickedNodes, mergedTextLen);
	}
	private getHighlightScriptIndex(scripts: Script[], selectedNodes: Node[], mergedTextLen: number) {
		const ret: number[] = [];
	...
	private async mounted() {
		...
		this.users = _.map(Object.keys(speechGroup), (key, userIndex) => {
			...
			for (let i = 0; i < this.seperateCount; i++) {
				const containSpeechs: Speech[] = _.filter(speechs, (speech) => {
					return config.speechGap * i < speech.index && speech.index < config.speechGap * (i + 1);
				});
		...
		this.users = this.makeUsersValidSpeechProperty(this.users, this.speakingBias);
		...
		for (let i = 0; i < this.seperateCount - 1; i++) {
			...
			const nextConnValue = connectionValue[i + 1] ? connectionValue[i + 1].prev : 0;
		...
		this.scriptsByTopic = this.getScriptsByTopic(this.contents, this.topicGroup);
		...
		_.forEach(this.users, (u, i) => {
			...
			// @ts-ignore
			u.path = line(path); // 138번째 줄 const line
		});
		//@ts-ignore
		this.$nextTick(() => {
			this.isMounted = true;
	...
	private getScriptsByTopic(speechContents: Speech[], topicGroup: any[]): Script[][] {
	...
	private makeTopicKeyword(
		...
		_.forEach(topicGroup, (topic, k) => {
			...
			for (let i = startIndex; i <= lastIndex; i++) {
				...
				for (let j = 0; j < this.networkData.topics.length; j++) {
					const exist: number = this.contents[i].text.indexOf(this.networkData.topics[j].label);
					if (exist !== -1) {
						//@ts-ignore
		...
		_.forEach(topicGroup, (topic, i) => {
			_.forEach(this.networkData.topics, (topicKeyword) => {
				// 꼴지만 아니면 넣는다.
				//@ts-ignore
				const min: number = _.min(_.values(topicIndexChooser[topicKeyword.label]));
				//@ts-ignore
				if (topicIndexChooser[topicKeyword.label][i] !== min || topicGroup.length <= 1) {
	...
	private moveUserSpeechPoint(user: User, from: number, to: number, std: number) {
